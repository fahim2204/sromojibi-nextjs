  ...
  try {
    ...
    const worker = await prisma.workerProfile.create({
      data: {
        ...
        phone: payload.phone,
        whatsapp_number: payload.whatsappNumber || payload.whatsapp_number || payload.whatsappPhone || null,
        secondary_phone: payload.secondaryPhone || payload.secondary_phone || null,
        gender: payload.gender || "MALE",
        age: payload.age ? (typeof payload.age === "number" ? payload.age : parseInt(String(payload.age).replace(/[^0-9]/g, "")) || null) : null,
        landmark: payload.landmark || payload.village || null,
        slug,
        workerCategories: {
