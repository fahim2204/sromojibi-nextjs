      ...
      <main className="min-h-screen bg-slate-50 text-slate-900 py-16 px-4">
        <div className="max-w-4xl mx-auto space-y-8">
          <div className="flex items-center gap-2 text-xs font-semibold text-emerald-600">
            ...
            <span>/</span>
            <span className="text-gray-600">{singleWorker.full_name}</span>
          </div>
          ...
          <div className="p-8 rounded-3xl bg-white border border-gray-200 space-y-6 shadow-sm">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-200 pb-6">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <h1 className="text-3xl font-black text-gray-900">{singleWorker.full_name}</h1>
                  {singleWorker.is_verified && (
                    <span className="px-2 py-0.5 rounded-md bg-emerald-100 text-emerald-800 border border-emerald-300 text-xs font-bold">
                      ✓ Verified Worker
                    </span>
                  )}
                </div>
              ...
              <div className="flex items-center gap-3">
                <a
                  ...
                  className="px-6 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold text-sm transition-all flex items-center gap-2 shadow-md"
                >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 py-2">
              <div className="p-4 rounded-2xl bg-slate-50 border border-gray-200">
                ...
                <p className="font-bold text-gray-900 text-base mt-1">{singleWorker.experience}</p>
              </div>
              <div className="p-4 rounded-2xl bg-slate-50 border border-gray-200">
                <p className="text-xs text-gray-500">City / Division</p>
                <p className="font-bold text-gray-900 text-base mt-1">{singleWorker.city}</p>
              </div>
              <div className="p-4 rounded-2xl bg-slate-50 border border-gray-200">
                <p className="text-xs text-gray-500">Rating</p>
                <p className="font-bold text-amber-600 text-base mt-1">
                  ★ {Number(singleWorker.rating ?? 5.0).toFixed(1)}
                </p>
              </div>
              <div className="p-4 rounded-2xl bg-slate-50 border border-gray-200">
                <p className="text-xs text-gray-500">Reviews</p>
                <p className="font-bold text-gray-900 text-base mt-1">{singleWorker.review_count} Customer Feedback</p>
              </div>
            </div>

            {singleWorker.details && (
              <div className="space-y-2 pt-2">
                <h3 className="font-bold text-gray-900 text-base">Services & Specialty Summary</h3>
                <p className="text-sm text-gray-700 leading-relaxed bg-slate-50 p-4 rounded-2xl border border-gray-200">
                  {singleWorker.details}
                </p>
              </div>
            )}
            ...
            <div className="pt-4 flex items-center justify-between border-t border-gray-200">
              <Link href="/workers" className="text-xs font-semibold text-gray-600 hover:text-emerald-600">
                ← Back to All Workers Directory
              </Link>
              <Link href="/join-worker" className="text-xs font-semibold text-emerald-600 hover:underline">
    ...
    <main className="min-h-screen bg-slate-50 text-slate-900 py-16 px-4">
      <div className="max-w-5xl mx-auto space-y-10">
        <div className="space-y-4">
          ...
          <h1 className="text-4xl sm:text-5xl font-black text-gray-900 flex items-center gap-3">
            <span>{icon}</span>
            <span className="capitalize text-emerald-600">{pageTitle}</span> Worker Listings
          </h1>
        ...
        {workers.length > 0 ? (
          ...
          <div className="p-8 rounded-3xl bg-white border border-gray-200 space-y-6 text-center shadow-sm">
            <div className="text-4xl">🛠️</div>
            <h2 className="text-2xl font-bold text-gray-900">Listings Initializing for {pageTitle}</h2>
